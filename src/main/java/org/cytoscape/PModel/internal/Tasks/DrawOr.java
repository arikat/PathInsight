...
public class DrawOr extends AbstractTask {
	...
	public DrawOr(CyApplicationManager appMgr, CyNetworkView netView, CyServiceRegistrar registrar, CyNetwork network, VisualMappingManager vmm) {
...
public void run(TaskMonitor task) {
		...
		if (cgProp == null) {
			...
			if (nodeTable.getColumn(col) != null) {
				network.getRow(node).set(col, "OR");
			}
		...
		public void run(TaskMonitor task) {
			...
			String lab = network.getRow(node).get(col, String.class);
				...
				if (lab.contains(hip)) {
					...
					netoView.getNodeView(node).setLockedValue(BasicVisualLexicon.NODE_FILL_COLOR, new Color(0x458b74, true));
			
					task.setStatusMessage("OR node parameterized.");
