...
public class DrawMacromolecule extends AbstractTask {
	...
	public DrawMacromolecule(CyApplicationManager appMgr, CyNetworkView netView, CyServiceRegistrar registrar, CyNetwork network, VisualMappingManager vmm) {
	...
	public void run(TaskMonitor task) throws Exception {
		...
		for (CyNode node : Nodes) {
			...
			netoView.getNodeView(node).setLockedValue(BasicVisualLexicon.NODE_SHAPE, NodeShapeVisualProperty.ROUND_RECTANGLE);
			netoView.getNodeView(node).setLockedValue(BasicVisualLexicon.NODE_FILL_COLOR, new Color(0xFFFFFF, false));
			
			task.setStatusMessage("replacing with macromolecule node...");
		}
