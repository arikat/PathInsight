...
public class DrawAnd extends AbstractTask {
	...
	public DrawAnd(CyApplicationManager appMgr, CyNetworkView netView, CyServiceRegistrar registrar, CyNetwork network, VisualMappingManager vmm) {
...
public void run(TaskMonitor task) {
		...
		if (cgProp == null) {
			...
			if (nodeTable.getColumn(col) != null) {
				network.getRow(node).set(col, "AND");
			}
		...
		public void run(TaskMonitor task) {
			...
			String lab = network.getRow(node).get(col, String.class);
				...
				if (lab.contains(hip)) {
					...
					netoView.getNodeView(node).setLockedValue(BasicVisualLexicon.NODE_SIZE, 20.0);
					netoView.getNodeView(node).setLockedValue(BasicVisualLexicon.NODE_SHAPE, NodeShapeVisualProperty.ELLIPSE);
					netoView.getNodeView(node).setLockedValue(BasicVisualLexicon.NODE_FILL_COLOR, new Color(0xff9999, true));
			
					task.setStatusMessage("AND node parameterized.");
